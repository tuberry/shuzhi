...
import { Fulu, ExtensionBase, Destroyable, manageSource, omit, getSelf, lightProxy, _ } from './fubar.js';
...
class ShuZhi extends Destroyable {
    ...
    _buildWidgets() {
        this._pts = [];
        this._src = manageSource(this, () => omit(this, 'systray', '_light'), {
    ...
    set refresh(refresh) {
        this._src.cycle.refreshSource(this._refresh = refresh);
    }

    set interval(interval) {
        this._interval = interval;
        if(this._refresh) this._src.cycle.refreshSource(true);
    }
